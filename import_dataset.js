...
function parseDataset() {
  ...
  const txList = []; // { date, account, reason, amount, type }

  // Opening balance row (Row index 1)
...
async function importData() {
  const targetEmail = 'dhanushg1020@gmail.com';
  const targetName = 'dhanush';
  const targetPassword = await bcrypt.hash('Dhanush@123', 10);

  console.log(`🚀 Importing dataset for ${targetEmail}...`);

  const parsedTxns = parseDataset();
  console.log(`📊 Parsed ${parsedTxns.length} transactions from the dataset.\n`);

  // 1. SUPABASE IMPORT
  console.log('☁️ Importing into Supabase...');
  ...
  if (existingSupUser) {
    supabaseUser = existingSupUser;
    console.log(`  ✓ Found Supabase user ID: ${supabaseUser.id}`);
  } else {
    const { data: newSupUser, error: userErr } = await supabase
      .from('users')
      .insert({
        name: targetName,
        email: targetEmail,
    ...
    supabaseUser = newSupUser;
    console.log(`  + Created Supabase user "${targetName}" (ID: ${supabaseUser.id})`);
  }

  // Ensure Accounts exist in Supabase (GPAY, CASH, FANPAY)
  const supabaseAccounts = {};
  for (const accName of ['GPAY', 'CASH', 'FANPAY']) {
    ...
    if (!acc) {
      const { data: newAcc, error: accErr } = await supabase
        .from('accounts')
        .insert({ name: accName, user_id: supabaseUser.id })
        .select()
        .single();
      if (accErr) throw accErr;
      acc = newAcc;
      console.log(`  + Created Supabase account "${accName}" (ID: ${acc.id})`);
    }
    supabaseAccounts[accName] = acc.id;
  }

  // Get categories from Supabase
  const { data: supCategories } = await supabase.from('categories').select('*');
  const getCatId = (reason) => {
    if (!reason || !supCategories) return null;
    ...
    if (r.includes('food') || r.includes('lunch') || r.includes('dinner') || r.includes('snack') || r.includes('chips') || r.includes('ice cream') || r.includes('cake') || r.includes('juice') || r.includes('juince') || r.includes('choculate')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('food'));
      return match ? match.id : null;
    }
    if (r.includes('bus') || r.includes('petrol') || r.includes('ride') || r.includes('travel')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('transport'));
      return match ? match.id : null;
    }
    if (r.includes('xerox') || r.includes('exam') || r.includes('nptel') || r.includes('hackaton') || r.includes('note') || r.includes('education')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('education'));
      return match ? match.id : null;
    }
    if (r.includes('mouse') || r.includes('stand') || r.includes('dress') || r.includes('socks') || r.includes('pant') || r.includes('shrit') || r.includes('shopping')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('shopping'));
      return match ? match.id : null;
    }
    if (r.includes('doctor') || r.includes('haircut') || r.includes('mask') || r.includes('health')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('healthcare'));
      return match ? match.id : null;
    }
    if (r.includes('cable') || r.includes('recharge') || r.includes('sms') || r.includes('aws') || r.includes('depit') || r.includes('annual')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('bills'));
      return match ? match.id : null;
    }
    if (r.includes('intern') || r.includes('reward') || r.includes('cash back') || r.includes('salary')) {
      const match = supCategories.find(c => c.name.toLowerCase().includes('salary'));
      return match ? match.id : null;
    }
    return null;
  };

  // Clear existing transactions for these accounts in Supabase
  const accountIds = Object.values(supabaseAccounts);
  await supabase.from('transactions').delete().in('account_id', accountIds);

  // Batch insert into Supabase
  const supabaseBatch = parsedTxns.map(tx => ({
    date: tx.date,
    account_id: supabaseAccounts[tx.account],
    category_id: getCatId(tx.reason),
    reason: tx.reason,
    amount: tx.amount,
    type: tx.type,
    created_at: new Date().toISOString()
  ...
  const batchSize = 50;
  for (let i = 0; i < supabaseBatch.length; i += batchSize) {
    const batch = supabaseBatch.slice(i, i + batchSize);
    const { error: insErr } = await supabase.from('transactions').insert(batch);
    if (insErr) throw insErr;
    console.log(`  + Uploaded batch ${Math.floor(i / batchSize) + 1} (${batch.length} rows) to Supabase.`);
  }

  // 2. LOCAL SQLITE IMPORT
  console.log('\n💾 Importing into local SQLite database (moneyflow.db)...');
  const dbPath = path.join(__dirname, 'moneyflow.db');
  const sqlite = new sqlite3.Database(dbPath);

  await new Promise((resolve, reject) => {
    sqlite.serialize(() => {
      // Find or insert user in SQLite
      sqlite.get('SELECT id FROM users WHERE email = ?', [targetEmail], (err, row) => {
        if (err) return reject(err);

        let sqliteUserId = row ? row.id : null;
        ...
        const onUserReady = (userId) => {
          ...
          ['GPAY', 'CASH', 'FANPAY'].forEach(accName => {
            sqlite.get('SELECT id FROM accounts WHERE name = ? AND user_id = ?', [accName, userId], (accErr, accRow) => {
              if (accRow) {
                sqliteAccounts[accName] = accRow.id;
                accountsLeft--;
                if (accountsLeft === 0) onAccountsReady(userId, sqliteAccounts);
              } else {
                sqlite.run('INSERT INTO accounts (name, user_id) VALUES (?, ?)', [accName, userId], function (insErr) {
                  sqliteAccounts[accName] = this.lastID;
                  accountsLeft--;
                  if (accountsLeft === 0) onAccountsReady(userId, sqliteAccounts);
                });
              }
        ...
        const onAccountsReady = (userId, sqliteAccounts) => {
          // Delete old transactions for these accounts
          const accIds = Object.values(sqliteAccounts);
          sqlite.run(`DELETE FROM transactions WHERE account_id IN (${accIds.join(',')})`, (delErr) => {
            if (delErr) return reject(delErr);

            // Fetch categories
            sqlite.all('SELECT * FROM categories', [], (catErr, cats) => {
              const stmt = sqlite.prepare('INSERT INTO transactions (date, account_id, category_id, reason, amount, type) VALUES (?, ?, ?, ?, ?, ?)');

              parsedTxns.forEach(tx => {
                const catId = getCatId(tx.reason);
                stmt.run(tx.date, sqliteAccounts[tx.account], catId, tx.reason, tx.amount, tx.type);
              });

              stmt.finalize((finErr) => {
                if (finErr) return reject(finErr);
                console.log(`  ✓ Inserted ${parsedTxns.length} transactions into local SQLite.`);
                resolve();
              });
            });
          });
        ...
        if (sqliteUserId) {
          onUserReady(sqliteUserId);
        } else {
          sqlite.run('INSERT INTO users (name, email, password) VALUES (?, ?, ?)', [targetName, targetEmail, targetPassword], function (insErr) {
            if (insErr) return reject(insErr);
            onUserReady(this.lastID);
          });
        }
      });
    });
  ...
  sqlite.close();
  console.log(`\n🎉 Successfully imported all transactions for ${targetEmail}!`);
}
